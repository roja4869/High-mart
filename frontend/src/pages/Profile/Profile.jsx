    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data && response.data.success) {
          ...
          if (!userObj.gender) userObj.gender = 'Male';
          if (!userObj.dob) userObj.dob = '1995-08-15';
          if (!userObj.bio) userObj.bio = 'Avid shopper & review contributor';
          setCurrentUser(userObj);
          setGlobalCurrentUser(userObj);
          fetchUserOrders(userObj.id);
        } else {
          loadLocalUser();
        }
      } catch (err) {
        console.warn("Failed to fetch profile from API. Using local user info.", err.message);
        loadLocalUser();
      }
    };
