...
const PRODUCT_IMAGE_MAP = {
  ...
  'Wallet': 'https://images.unsplash.com/photo-1568252542512-9fe8fe9c87bb?w=600&q=80', // corrected typo here in wallet key if any
...
async function verify() {
  try {
    ...
    const sortedKeys = Object.keys(PRODUCT_IMAGE_MAP).sort((a, b) => b.length - a.length);

    for (const p of products) {
      if (brokenUrlsList.includes(p.image)) {
        console.error(`[BROKEN] Product ${p.id} (${p.name}) still has broken URL: ${p.image}`);
        brokens++;
      }

      // Check if it should match a noun
      let expectedImage = null;
      for (const key of sortedKeys) {
        if (p.name.includes(key) || p.name.endsWith(key)) {
          expectedImage = PRODUCT_IMAGE_MAP[key];
          break;
        }
      }

      // We only enforce match for ID >= 13 or specific name Cashew
      if (p.id >= 13 || p.name.toLowerCase() === 'cashew') {
        if (expectedImage && p.image !== expectedImage) {
          console.error(`[MISMATCH] Product ${p.id} (${p.name}) image is ${p.image}, expected: ${expectedImage}`);
          mismatches++;
        }
      }
    }
    ...
    console.log(`Broken URLs found: ${brokens}`);
    console.log(`Mismatches found: ${mismatches}`);
    
    if (brokens === 0 && mismatches === 0) {
      console.log("SUCCESS: All products are correctly mapped and no broken URLs remain!");
    } else {
      console.log("FAILURE: Mismatches or broken URLs exist. Check log details above.");
    }
    process.exit(0);
  } catch (err) {
